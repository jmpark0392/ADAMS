...
public class EgovConfigAppDatasource {
	...
	void init() {
		...
		//Exception 처리 필요
		className = env.getProperty("Globals.DriverClassName");
		url = env.getProperty("Globals.Url");
		userName = env.getProperty("Globals.UserName");
		password = env.getProperty("Globals.Password");
		log.debug(className);
		log.debug(url);
		log.debug(userName);
		log.debug(password);
